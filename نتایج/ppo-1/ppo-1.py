...
class WaterNetworkEnv:
    ...
    def _reward_function(self, node_pressures, clip_min=-100.0, clip_max=100.0,
                         min_pressure=40.0, max_pressure=60.0):
        pressures = np.array(node_pressures, dtype=np.float32)
        avg_pressure = float(np.mean(pressures))

        # (الف) جریمه برای فشار خارج از بازه [40, 60]
        under_penalty = np.sum(min_pressure - pressures[pressures < min_pressure])
        over_penalty = np.sum(pressures[pressures > max_pressure] - max_pressure)

        # (ب) پاداش پایه: هر نود داخل بازه 40–60 امتیاز مثبت می‌گیره
        in_range = np.logical_and(pressures >= min_pressure, pressures <= max_pressure)
        reward = np.sum(in_range) * 10.0  # ۱۰ امتیاز برای هر نود در بازه

        # (ج) جریمه‌ها
        reward -= 2.0 * under_penalty + 1.0 * over_penalty

        # (د) جریمه برای واریانس بالا (برای یکنواختی فشارها)
        reward -= 0.1 * float(np.std(pressures))

        # (هـ) نرمال‌سازی بر اساس تعداد نودها
        reward /= len(pressures)

        # (و) محدود کردن برای پایداری
...
if __name__ == "__main__":
    inp_file = "/content/GES4-9.inp"   # <- مسیر فایل .inp خودت را اینجا قرار بده
