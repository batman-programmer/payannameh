...
class WaterNetworkEnv:
    ...
    def _reward_function(self, node_pressures, target_pressure=50.0, clip_min=-100.0, clip_max=100.0):
        ...
        # (الف) پاداش پایه: هرچه فشارها به target نزدیک‌تر، بهتر
        reward = 100.0 - mad

        # (ب) جریمه برای واریانس بالا
        reward -= 0.1 * float(np.std(pressures))

        # (ج) جریمه فشارهای خارج از محدوده [30, 100]
        if np.any(pressures < 30):
            reward -= np.sum(30 - pressures[pressures < 30]) * 2.0
        if np.any(pressures > 100):
            reward -= np.sum(pressures[pressures > 100] - 100) * 1.0

        # (د) پاداش اضافه برای خیلی نزدیک بودن به هدف (MAD < 2 متر)
        if mad < 2:
            reward += 20.0

        # (هـ) نرمال‌سازی بر اساس تعداد گره‌ها
        reward /= len(pressures)

        # (و) محدود کردن برای پایداری یادگیری
