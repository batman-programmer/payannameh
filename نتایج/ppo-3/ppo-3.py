...
class WaterNetworkEnv:
    ...
    def _reward_function(self, node_pressures, target_pressure=50.0, clip_min=-100.0, clip_max=100.0):
        ...
        # پاداش پایه با حساسیت بیشتر (تابع نمایی)
        reward = np.exp(-0.1 * mad) * 100.0

        # جریمه برای واریانس بالا
        reward -= 0.1 * float(np.std(pressures))

        # جریمه فشارهای خارج از محدوده [30,100]
        if np.any(pressures < 30):
            reward -= np.sum(30 - pressures[pressures < 30]) * 5.0
        if np.any(pressures > 100):
            reward -= np.sum(pressures[pressures > 100] - 100) * 3.0

        # پاداش اضافی برای نزدیک بودن میانگین به هدف ±5 متر
        if abs(avg_pressure - target_pressure) <= 5:
            reward += 50.0

        # نرمال‌سازی بر اساس تعداد گره‌ها
        reward /= len(pressures)

        # محدود کردن برای پایداری یادگیری
